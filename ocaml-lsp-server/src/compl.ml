...
let ident_suffix_of_position source position =
  suffix_of_position ~is_char:ident_char source position
;;

let reconstruct_ident source position =
  let prefix = prefix_of_position ~short_path:false source position in
  let suffix = ident_suffix_of_position source position in
...
module Complete_by_prefix = struct
  ...
  let complete doc prefix pos ~position ~position_encoding ~deprecated ~resolve =
